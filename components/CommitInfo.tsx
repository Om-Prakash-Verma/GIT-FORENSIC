...
const CommitInfo: React.FC<CommitInfoProps> = ({ commit, analysis, loading, onAnalyze }) => {
  ...
  return (
    <div className="w-full h-full border-l border-slate-800/60 bg-[#020617] flex flex-col overflow-y-auto custom-scrollbar">
        ...
        <div className="space-y-6">
          ...
          {analysis ? (
            <div className="space-y-6 lg:space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
              ...
              <div className="bg-black border border-slate-800 p-4 lg:p-5 rounded-2xl relative overflow-hidden">
                ...
                <div className="flex items-end justify-between mb-4 relative z-10">
                  ...
                  <div className={`px-2 py-0.5 rounded text-[8px] lg:text-[9px] font-black uppercase ${getScoreColor(analysis.probabilityScore)} border currentColor`}>
                ...
                <div className="w-full bg-slate-900 h-1.5 rounded-full overflow-hidden mb-2">
                  <div 
                    className={`h-full transition-all duration-1000 ease-out ${getScoreBg(analysis.probabilityScore)}`}
            ...
            <div className="py-8 lg:py-12 flex flex-col items-center justify-center text-center px-4">
              ...
              <p className="text-[10px] lg:text-xs font-black uppercase tracking-widest text-slate-500 mb-6">Forensic Data Ready</p>
              
              <button 
                onClick={onAnalyze}
                disabled={!commit || commit.diffs.length === 0}
                className="w-full py-4 bg-amber-500 hover:bg-amber-400 text-black font-black text-[10px] uppercase tracking-[0.2em] rounded-xl transition-all shadow-lg active:scale-95 disabled:opacity-30 flex items-center justify-center gap-3"
              >
      ...
      {/* Mobile-only spacer for bottom tabs if needed */}
